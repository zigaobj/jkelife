...
//	SPI1_RWReg((REG_WRITE | MicrocontrollerOutputClock), 0x05);				//GPIO输出2M时钟
	...
	SPI1_RWReg((REG_WRITE | ModulationModeControl1),0x20);							//不使用曼彻斯特编码
  ...
  //case RATE_24K: // 2.4k 
	...
	SPI1_RWReg((REG_WRITE | AFCLoopGearshiftOverride), 0x00);						//禁止AFC
	//频率设置													
	SPI1_RWReg((REG_WRITE | FrequencyBandSelect), 0x53);    						//(75H)边带选择，低频段240-479.9M 430Mhz
	SPI1_RWReg((REG_WRITE | NominalCarrierFrequency1), 0x4b);  					//433HZ
	SPI1_RWReg((REG_WRITE | NominalCarrierFrequency0), 0x00);

	SPI1_RWReg((REG_WRITE | FrequencyDeviation), 0x20);                 //72h 频率偏差20k                            
	SPI1_RWReg((REG_WRITE | FrequencyOffset),0x00); 


	SPI1_RWReg((REG_WRITE | ModulationModeControl1), 0x00); 
	SPI1_RWReg((REG_WRITE | ModulationModeControl2), 0x22); 						//直接模式 FSK调制

	SPI1_RWReg((REG_WRITE | HeaderControl2), 0x02); //no head; sync word 3 and 2
	SPI1_RWReg((REG_WRITE | HeaderControl1), 0x00);
	SPI1_RWReg((REG_WRITE | PreambleLength), 0x04);   // 2 byte
	SPI1_RWReg((REG_WRITE | PreambleDetectionControl), 0x10); //8bit
	SPI1_RWReg((REG_WRITE | SyncWord3), 0x2d);
	SPI1_RWReg((REG_WRITE | SyncWord2), 0xa4);
	SPI1_RWReg((REG_WRITE | DataAccessControl), 0x88); //enable TX handling
	SPI1_RWReg((REG_WRITE | IFFilterBandwidth), 0x03); //BW=90kHZ
	SPI1_RWReg((REG_WRITE | ClockRecoveryOversamplingRatio), 0xa1);
	SPI1_RWReg((REG_WRITE | ClockRecoveryOffset2), 0x20);
	SPI1_RWReg((REG_WRITE | ClockRecoveryOffset1), 0x4e);
	SPI1_RWReg((REG_WRITE | ClockRecoveryOffset0), 0xa5);
	SPI1_RWReg((REG_WRITE | ClockRecoveryTimingLoopGain1), 0x00);
	SPI1_RWReg((REG_WRITE | ClockRecoveryTimingLoopGain0), 0x36);
	SPI1_RWReg((REG_WRITE | TXDataRate1), 0x02); //9600BPS
	SPI1_RWReg((REG_WRITE | TXDataRate0), 0x75);
	
	SPI1_RWReg((REG_WRITE | ClockRecoveryGearshiftOverride), 0x13);

	SPI1_RWReg((REG_WRITE | OperatingFunctionControl2), 0x03);
	SPI1_RWReg((REG_WRITE | OperatingFunctionControl2), 0x00);
	SPI1_RWReg((REG_WRITE | OperatingFunctionControl1), 0x01);		//ready模式，Xtal
	SPI1_RWReg((REG_WRITE | InterruptEnable1), 0x02);		//使能接收中断
	SPI1_RWReg((REG_WRITE | TXPower), 0x07);
        
	SPI1_RWReg((REG_WRITE | 0x09), 0x3f);
}
